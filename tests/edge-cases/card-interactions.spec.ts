...
test.describe('Edge Cases and Error States', () => {
  test('Card Interaction Edge Cases', async ({ page }) => {
    ...
    await expectCardFaceUp(cards.nth(0), true);
    await expectCardFaceUp(cards.nth(9), true);
    await expect(page.getByTestId('score-blue')).toHaveText(initialBlueScore || '1');
    await expect(page.getByTestId('score-red')).toHaveText(initialRedScore || '0');
    await expect(page.getByText("Blue Player's Turn")).toBeVisible();
    ...
    const initialFaceUpCount = await cards.filter({ has: page.locator('[data-face-up="true"]') }).count();
    ...
    const currentFaceUpCount = await cards.filter({ has: page.locator('[data-face-up="true"]') }).count();
    ...
    const faceUpCountDuringCheck = await cards.filter({ has: page.locator('[data-face-up="true"]') }).count();
