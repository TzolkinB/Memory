...
test.describe('Game Controls and Restart', () => {
  test('Restart Functionality', async ({ page }) => {
    ...
    await expect(page.getByText('Are you sure you want to reshuffle and restart the game?')).toBeVisible();
    ...
    await expect(page.getByText('Are you sure you want to reshuffle and restart the game?')).not.toBeVisible();
    await expect(page.getByTestId('score-red')).toHaveText('1');
    await expect(page.getByText("Red Player's Turn")).toBeVisible();
